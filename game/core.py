...
class Simulation:
                ...
                Result(
                    ...
                    dist_to_next_cp=dist_between(
                        ...
                        self.track.cps_arr[
                            (
                                car.score
                                + self.track.spawn_index
                                + 1 % len(self.track.cps_arr)
                            )
                        ],
                    ),
                )
    ...
    def get_cars_sorted(self):
        return sorted(
            self.cars, key=lambda car: (car.score, -car.dist_to_cp), reverse=True
        )
        ...
        for sen_ind in range(car.sensors.shape[0]):
            ...
            for plus in self.checkpoint_range:
                ...
                for line in lines:
                    ...
                    intersection = find_intersection(*sen_pos, *line)
                    if intersection:
    ...
    def behave(self, dt):
        ...
        for car in self.cars:
            if car.active == True:
                ...
                car.turn((out[0] - 0.5) * 2)  # number between -1 and 1
        # if no car is active :(
        if inactive:
            return False
        else:
    ...
    def __init__(self, nodes, shape, spawn_index=0, spawn_angle=None, bg=False):
        ...
        self.vertex_lists = (
            pyglet.graphics.vertex_list(
                len(self.nodes[0]), ("v2i", (self.nodes[0].flatten()).astype(int))
            ),
            pyglet.graphics.vertex_list(
                len(self.nodes[1]), ("v2i", (self.nodes[1].flatten()).astype(int))
            ),
        )
        self.bg = bg
...
def move_in_map(MAP, x, y, heading, directions):
    """
    MAP: a matrix containing our landmarks
    x: the initial x-coordinate
    y: the initial y-coordinate
    heading: the initial orientation
    ...
    for direction in directions:
        ...
        visited_cells.append(MAP[j][i])

    return visited_cells


MAP = [["A", "L"], ["B", "I"], ["C", "H"], ["D", "G"], ["E", "F"]]
